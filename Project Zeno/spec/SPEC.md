...
**Version:** 1.6.0
**Phase scope:** Phase 1 (single bonded executor per domain; bonded attestation, not trustless execution) is the single **active** point of a general model. The domain schema is generalised for multi-domain, multi-executor, external-input, and **bridge/messaging** operation across two per-domain security ladders (execution + foreign-fact) and two custody modes (§6, §17, §18), with Phase 1 as the degenerate case and every other configuration reserved/gated. The generalized bridge framework's reference material (worked examples, comparative mappings, testing/deployment) is the companion `research/bridges/BRIDGE-FRAMEWORK-SPEC.md`.
**Classification:** Normative requirements with *Informative* commentary and explicitly marked *Deferred* items.

**Revision history:** maintained in `CHANGELOG.md`.
...
> **Phase 1 activates a single point on a general trust surface: bonded attestation with no external inputs. It is not trustless execution.** This is normative context for the entire specification and **MUST** be communicated to users and integrators without qualification or obfuscation.

A domain's trust is defined by two **independent** profile dimensions, each a ladder that strengthens additively (§6.3, §6.4). Phase 1 occupies the base rung of each:

- **Execution profile** — how a batch's state transition is secured: `ATTESTATION` (bonded, no on-chain correctness check — Phase 1) → `OPTIMISTIC` (fraud proofs) → `ZK_VALIDITY` (validity proofs). This is the Phase 1→2→3 ladder (§2.2) lifted to a per-domain field.
- **Foreign-fact profile** — how the truth of an external-chain event consumed as input is established: `NONE` (no external input — Phase 1) → `COMMITTEE` (attester quorum) → `LIGHT_CLIENT` (verified in the STF) → `ZK_CONSENSUS` (succinctly proven). Only `BRIDGE`/`MESSAGING` domains use a non-`NONE` value (§6).

**A domain is exactly as trustless as the weaker of its two profiles, and no more.** There is no single global trust assumption; each domain carries its own pair and its own trust model, which **MUST** be surfaced to users.

Three permanent safety rails hold in every profile:

- **Rail 1 — STF-verifiable is not Settlement-verified.** Under `ATTESTATION`, Settlement records `preStateRoot`/`postStateRoot` but **MUST NOT** verify them; correctness rests on the bonded executor. Execution is on-chain-verified only under `ZK_VALIDITY`, or on challenge under `OPTIMISTIC`.
- **Rail 2 — a foreign fact commits what was observed, not what is true.** A commitment binds the inputs the executor consumed; whether those reflect a real foreign event depends on the foreign-fact profile.
- **Rail 3 — commitment is not availability.** `DAHash` commits a bundle; it does not guarantee retrievability (§20). An unavailable bundle reduces the affected batch to executor honesty regardless of profile.

The chain cannot pay out more than was deposited/backed in aggregate per (domain, asset) — enforced on-chain, independent of executor honesty for the `ESCROW` custody mode; for `MINTED` the guarantee is profile-dependent (§18.3). Per-account balance correctness relies on the bonded executor's honesty until the execution profile removes it.

| Guaranteed on-chain (executor-independent) | Established by the profile, not free |
|---|---|
| Aggregate ESCROW solvency: `totalReleased + pendingWithdrawalReserve ≤ totalDeposited` | Per-account / per-message correctness (executor honesty until `OPTIMISTIC`+) |
| Domain isolation: an executor can never reach another domain's custody | On-chain verification of `preStateRoot`/`postStateRoot` (only `ZK_VALIDITY`, or `OPTIMISTIC` on challenge) |
| Canonical input order fixed by L1 (`L1_NATIVE`/`L1_RELAYED`); no reorder/skip/insert | Truth of a foreign fact (the foreign-fact profile) |
| Batch chaining, contiguous ranges, finality window, per-batch caps, public commitments | Aggregate MINTED solvency (profile-dependent, §18.3) and DA availability |

**Normative:**

- The Settlement contract **MUST NOT** execute any STF, and **MUST NOT** verify execution correctness except where the execution profile requires it (a validity proof under `ZK_VALIDITY`; a fraud proof under `OPTIMISTIC`).
- The Settlement contract **MUST** record batch commitments and **MUST** enforce custody, finality, batch caps, and the per-(domain, asset) conservation invariant (§18).
- Aggregate over-withdrawal (`ESCROW`) and over-mint / over-release (`MINTED`, at its profile floor) **MUST** be prevented on-chain by Settlement conservation accounting (§18), within the guarantees each custody mode's profile provides.
- **Phase 1 active point (normative):** `execProfile = ATTESTATION`, `foreignProfile = NONE`, `custodyMode = ESCROW`. Every other value is reserved and Core **MUST** reject it while the Phase-1 profile is in force (§6.1, §6.5). At this point the safety mechanisms are exactly the bond, the withdrawal delay, the per-batch cap, the conservation invariant, and public commitment visibility.

> The honest framing: at the Phase-1 point the chain cannot pay out more than was deposited in aggregate (enforced on-chain), but within the withdrawal delay and bounded by the bond and the per-batch cap it can misattribute balances among accounts if the executor is dishonest. That residual trust is removed by `OPTIMISTIC` (fraud proofs) and `ZK_VALIDITY` (validity proofs); the foreign-fact profile removes the analogous trust on external inputs. The generalized bridge model layered on this base — profiles, custody modes, and the per-chain seams — is normative here (§6, §17, §18); its worked examples, comparative mappings, and testing/deployment strategy live in the companion `research/bridges/BRIDGE-FRAMEWORK-SPEC.md`.
...
- **Settlement assigns the input sequence on-chain.** Every account-block that decodes to a valid `{domainId, payload}` input envelope for a registered domain (§4.1) **MUST** be assigned the next global input index by Settlement at its embedded receive and recorded against its domain, together with per-(domain, asset) cumulative deposit totals (§18.4). A send that does not decode to a valid envelope is not an input and consumes no index (the embedded receive fails; any attached value is refunded by the standard embedded rollback) — the same predicate the executor applies when deriving §4.5, so the two numberings coincide. Settlement **MUST** verify batch bounds against its own assignment: `firstInputSeq` **MUST** be the domain's earliest assigned-but-unconsumed index; `lastInputSeq` **MUST** be an index assigned to that domain and **MUST NOT** exceed the highest index Settlement has assigned; and the count of the domain's assigned indices in `[firstInputSeq, lastInputSeq]` **MUST NOT** exceed `MaxBatchInputs`.

> *Informative.* Embedded receives replay the committed send order — the per-contract sequencer is FIFO over `MomentumContent` order (`chain/account/sequencer.go`, consumed by `pillar/worker_contract_generator.go`) — so the receive-time assignment reproduces the §4.5 derivation exactly. This also closes the receive-lag race: a batch covering inputs whose receives have not yet been processed refers to indices Settlement has not assigned and is rejected; the executor resubmits once the receives land.
...
|---|---|---|---|
| `RegisterDomain` | Admin | core | Register/configure a domain: `domainClass`, `runtimeKind`, `stfSpecHash`, `inputSource`, `execProfile`, `foreignProfile`, `profileConfig`, `chainBinding`, `minExecutors`, `valueCaps`, status (§6). Phase 1 registers exactly one: a WASM `EXECUTION` domain at the base profile point |
| `RegisterExecutor` | Admin | core | Bind/replace the executor identity for a `domainId` (§22) |
| `DepositQsr` / `WithdrawQsr` | Executor | core | Accumulate / reclaim the QSR leg of the executor bond (sentinel pattern, §22) |
| `PostBond` | Executor | core | Post the ZNN leg; consumes the required QSR deposit and activates the bond (§22) |
| `RevokeBond` | Former executor | core | Reclaim both bond legs once replaced and all own batches are `FINALIZED` (§22) |
...
| `RelayMessage` | Permissionless | core | Relay a proven, finalized `domainId` outbox message; registers L1 withdrawals for release (§17, §18.2) |
| `Update` | Permissionless | core | Rate-limited maintenance crank: releases due withdrawals from the domain's release queue, ≤ `MaxWithdrawalsPerUpdate` per call (§18.2, §27.9) |
...
- The global received-input counter and, per domain, the ordered record of assigned input indices with per-(domain, asset) cumulative deposit totals (§4.6, §18.4).
- The per-domain withdrawal release queue: registered, not-yet-released withdrawals in registration order (§18.2).
- Executor registration and bond per domain, and per-address QSR bond-deposit balances (`DepositQsr` accumulation, §22).
- **(reserved, gated)** The per-(domain, asset) `AssetRegistry`: `custodyMode` (`ESCROW`/`MINTED`) and, for `MINTED`, the `(foreignChainId, foreignAssetRef, decimalsPolicy)` binding and the counters `provenForeignLocks`, `mintedSupply`, `burnedSupply`, `authorizedForeignRelease` (§18.3). Absent/`ESCROW`-only while the Phase-1 profile is in force.
- **(reserved, gated)** Per-`outboxId` `MessageState` for outbox messages carrying a timeout/ack policy (§17.4); fire-and-forget messages need only `processedOutbox` membership.
- **(reserved, gated)** Periphery verifier registries: fraud-referee entries for `OPTIMISTIC` (§6.3) and verifying-key entries for `ZK_VALIDITY`/`ZK_CONSENSUS` (§6.3, §6.4). The registry entry format and the Core→verifier dispatch interface are an OPEN DESIGN item (§30).
...
Settlement anchors **domains**; a domain is the unit of runtime + STF + input source + execution profile + foreign-fact profile + bonded executor set + state-root lineage + optional foreign-chain binding. **Phase 1 registers exactly one domain: a WASM `EXECUTION` domain at the base profile point** (`ATTESTATION` / `NONE` / `ESCROW` / `SINGLE`, §1). The schema below is the general model; Phase 1 is its degenerate case, so no migration is needed to add profiles, bridges, or messaging later.

A domain carries a `domainClass` — `EXECUTION` (general computation, e.g. WASM), `BRIDGE` (moves value to/from a foreign chain, with custody), or `MESSAGING` (moves cross-chain data only, no custody). `EXECUTION` is the only class activated in Phase 1; `BRIDGE`/`MESSAGING` are reserved (§6.5).
...
DomainRecord {
    ...
    domainClass     // EXECUTION | BRIDGE | MESSAGING — informative classification (§6.5)
    runtimeKind     // WASM — the only kind with a handler in Phase 1
    stfSpecHash     // pins runtimeKind, VM version, genesis, input-envelope + commitment format,
                    //   AND domainClass/inputSource/execProfile/foreignProfile/chainBinding (§24)
    inputSource     // L1_NATIVE | L1_RELAYED | EXTERNAL_OBSERVED — where inputs originate (§6.1)
    execProfile     // ATTESTATION | OPTIMISTIC | ZK_VALIDITY — secures the batch STF (§6.3)
    foreignProfile  // NONE | COMMITTEE | LIGHT_CLIENT | ZK_CONSENSUS — secures foreign facts (§6.4)
    profileConfig   // tagged union carrying each profile's parameters (§6.3, §6.4)
    chainBinding    // ChainBinding | null — non-null iff domainClass ∈ {BRIDGE, MESSAGING} (§6.6)
    ...
    finalityModel   // DELAY | CHALLENGE_WINDOW | PROOF_VERIFIED — derived from execProfile (§6.3, §21)
    status          // active / paused
}

ChainBinding {                 // present only for BRIDGE / MESSAGING domains (§6.6)
    foreignChainId    // canonical foreign-chain identifier
    chainVerifierId   // pins the ChainVerifier implementation (foreign-fact verification, §6.6)
    genesisAnchor     // light-client checkpoint: header / sync-committee root / bank hash
    releaseAdapterRef // foreign-side ReleaseAdapter identifier (addr / script / graph id, §6.6)
    finalityParams    // per-chain confirmation depth / finality predicate (pre-mainnet, §6.4)
}
```

> **Settlement Core change (gated).** `domainClass`, `execProfile`, `foreignProfile`, `profileConfig`, `chainBinding`, and `finalityModel` are semantics-defining Core additions folded into `stfSpecHash` (§24); activating any non-Phase-1 value requires a spork-gated `znnd` release. Until then Core **MUST** constrain them to the Phase-1 defaults below and reject any other value.

**Normative:**

- `RegisterDomain` is **admin-only** (the Settlement administrator, §23) in Phase 1 and registers exactly one domain (`domainClass = EXECUTION`, `runtimeKind = WASM`, `inputSource = L1_NATIVE`, `execProfile = ATTESTATION`, `foreignProfile = NONE`, `chainBinding = null`, an `executors` set of size 1, `proposerPolicy = SINGLE`). `domainClass`, `inputSource`, `execProfile`, `foreignProfile`, `chainBinding`, `executors`, and `proposerPolicy` are fixed at registration and changeable only by the governed mechanisms of §6.1–§6.6 (a change to any semantics-defining field is a runtime-semantics change and **MUST** be treated as a `stfSpecHash` bump under `MinRuntimeUpgradeDelay`).
...
### 6.3 Execution-profile ladder (secures the batch STF)

`execProfile` determines how Settlement treats `preStateRoot`/`postStateRoot`, what `SubmitBatch` verifies, the `finalityModel`, and the contents of the reserved `proofData` field (§19). It is the Phase 1→2→3 ladder of §2.2 lifted to a per-domain field.

```
profileConfig (execProfile arm) :=
  ATTESTATION  { attesterPolicy }                    // single-bonded (Phase 1) or quorum (reserved)
  OPTIMISTIC   { challengeWindow ≥ WithdrawalDelay,  // heights
                 fraudVerifierRef }                  // referee registry entry (§5.4)
  ZK_VALIDITY  { validityVerifierRef, verifyingKeyId, publicInputSchema }
```

| `execProfile` | `SubmitBatch` verifies | `proofData` carries | `finalityModel` | On-chain correctness |
|---|---|---|---|---|
| `ATTESTATION` | proposer entitlement, contiguity, size, `assetFlowSummary`, chaining — **not execution** | empty (single-bonded, Phase 1) | `DELAY` (§21) | none (Rail 1) |
| `OPTIMISTIC` | as above, and opens a challenge window | empty at submit; a fraud proof is supplied later via `Challenge` | `CHALLENGE_WINDOW` | on challenge: the referee re-runs the §13.5.1 applier for the single disputed input and, on success, rejects the batch and slashes the bond |
| `ZK_VALIDITY` | as `ATTESTATION`, plus verifies a validity proof against `verifyingKeyId` before finalization | the SNARK over the batch transition | `PROOF_VERIFIED` (**MAY** be immediate) | full (the proof) |

**Normative:**
- A single batch transition **MUST** satisfy `postStateRoot = F(preStateRoot, input_data, code_hash)` in every profile; the profile changes only *who checks it and when*, never the function (§17.1). The fraud-proof referee **MUST** re-run the identical canonical applier (§13.5.1) including the deposit-settlement step (§18.5), so "the executor ran the wrong code/version" is ordinary state divergence, not a special Settlement failure mode.
- Under `ZK_VALIDITY`, Settlement **MUST** verify the validity proof before a batch finalizes and **MUST** reject a batch whose proof does not verify against the registered `verifyingKeyId`. Under `OPTIMISTIC`, Settlement **MUST** verify a submitted fraud proof by the referee procedure and, on success, **MUST** reject the challenged batch and slash the proposer's bond.
- `proofData` **MUST** be the zero-length encoding under `ATTESTATION`-single-bonded and under `OPTIMISTIC` at submit time; Core **MUST** reject a non-empty `proofData` outside the profiles/phases that define its contents (§19, §24).
- **Phase 1:** only `ATTESTATION` (single-bonded) **MAY** be activated; `OPTIMISTIC` requires the fraud-proof referee registry and `ZK_VALIDITY` the validity-verifier registry (§5.4), both reserved. `finalityModel` **MUST** equal the value mandated by `execProfile`; Core **MUST** reject a `RegisterDomain` whose `finalityModel` disagrees.

### 6.4 Foreign-fact profile ladder (secures external-chain truth)

For `BRIDGE` and `MESSAGING` domains, `foreignProfile` determines how the `ChainVerifier` (§6.6) establishes that a foreign event happened and is final. This ladder is **orthogonal** to §6.3: it secures the *truth of a foreign fact consumed as input*, not the *correctness of the batch's computation*.

```
profileConfig (foreignProfile arm) :=
  NONE         { }                                   // non-bridge EXECUTION domains
  COMMITTEE    { attesterSet, threshold }            // relayer/oracle quorum
  LIGHT_CLIENT { clientKind, finalityRule }          // PoW | sync-committee | tower-bft | … (per chain)
  ZK_CONSENSUS { consensusVerifierRef, verifyingKeyId }
```

| `foreignProfile` | Foreign truth established by | Tolerable adversary |
|---|---|---|
| `COMMITTEE` | a quorum of foreign attesters signs the event; Core or the STF checks the threshold signature | `< threshold` colluding attesters |
| `LIGHT_CLIENT` | foreign consensus + inclusion verified from relayed headers/proofs inside the STF | `< foreign safety threshold` |
| `ZK_CONSENSUS` | a succinct proof of foreign consensus + inclusion | `< foreign safety threshold` + proof soundness |

**Normative:**
- `foreignProfile = NONE` iff `domainClass = EXECUTION`.
- Under `LIGHT_CLIENT` and `ZK_CONSENSUS`, the `ChainVerifier` **MUST** reject any relayed datum that does not extend the verified light-client view; a dishonest relayer **MUST** be unable to advance the foreign view to a state the foreign chain did not reach (Rail 2).
- `finalityRule` and `chainBinding.finalityParams` are pre-mainnet per chain and **MUST** be chosen so that no Zenon-side release ever finalizes over a re-organizable foreign event (§21).
- **Phase 1:** only `NONE` **MAY** be activated; `COMMITTEE`/`LIGHT_CLIENT`/`ZK_CONSENSUS` are reserved.

> The two ladders compose. A Bitcoin bridge is naturally `(execProfile = OPTIMISTIC, foreignProfile = LIGHT_CLIENT)` — the BTC lock is SPV-verified in the STF while the batch computation is fraud-proven. An Ethereum bridge is `(OPTIMISTIC | ZK_VALIDITY, LIGHT_CLIENT)`; a high-assurance target is `(ZK_VALIDITY, ZK_CONSENSUS)`. Settlement mandates no pairing; §6.5 constrains only the legal combinations.

### 6.5 Domain classes, legal combinations, and custody modes

`domainClass` classifies a domain (informative, but load-bearing for the rules below): `EXECUTION` runs general computation; `BRIDGE` moves value across a foreign-chain boundary with custody; `MESSAGING` moves cross-chain data only, with no custody. Each `(domain, asset)` also carries a **custody mode** — `ESCROW` (a real ZTS held 1:1 in Settlement custody, §18.1) or `MINTED` (a representation asset backed by attested foreign locks, §18.3).

**Normative:** Core **MUST** reject a `RegisterDomain` violating any of:
1. `chainBinding = null` ⟺ `domainClass = EXECUTION` ⟺ `foreignProfile = NONE`.
2. `MESSAGING` domains **MUST** define no `custodyMode` for any asset and **MUST NOT** emit `kind=1` withdrawal authorizations (§17); they emit only `kind=0` cross-domain/cross-chain messages.
3. `inputSource = EXTERNAL_OBSERVED` **MUST** be paired with `foreignProfile ∈ {LIGHT_CLIENT, ZK_CONSENSUS}` or an explicitly disclosed `COMMITTEE` predicate; an `EXTERNAL_OBSERVED` domain whose observation reduces to "the executor says so" with no foreign-fact profile **MUST NOT** be activated (it would violate Rail 2 with no compensating verification).
4. `custodyMode = MINTED` **MUST** satisfy the §18.3 profile floor (`execProfile ≥ OPTIMISTIC`, or Core-verifiable `foreignProfile`), and **MUST NOT** be activated until the `MINTED` reconciliation mechanism is defined (§18.3, OD-1).
5. `execProfile = OPTIMISTIC` requires the fraud-proof referee registry; `execProfile = ZK_VALIDITY` requires the validity-verifier registry (§5.4). Core **MUST** reject either until its registry entry exists.

**Migration** along either ladder is a `stfSpecHash` bump under `MinRuntimeUpgradeDelay ≥ WithdrawalDelay`, is a **strict strengthening**, and changes no commitment format and no custody accounting: the commitment already carries `proofData` (empty → fraud-proof refs → validity proof) and `executorId` (single → set member), so no on-chain footprint changes except added Periphery verifier registries. A migration **MUST NOT** alter any finalized root, receipt, or conservation counter, and the replacement's first batch `preStateRoot` **MUST** equal the last accepted `postStateRoot`.

### 6.6 Bridge abstractions and the peg-out authorization

A `BRIDGE`/`MESSAGING` domain is still an ordinary domain served by the executor's four-method plugin (`EXECUTOR.md` §7); the bridge-specific logic lives entirely inside its STF and in two per-chain seams. Settlement is unaware which class a domain is — it anchors batches and enforces custody/conservation identically (Rail-independent, §3).

- **`ChainVerifier` (read seam, foreign → Zenon).** The per-chain foreign-fact verifier that implements `foreignProfile` inside the STF: verifies foreign headers/finality and event inclusion. Pure and deterministic (no live foreign reads inside the STF — `EXECUTOR.md` §10). Defined in `EXECUTOR.md` §7.
- **`SourceAdapter` (relay seam).** The per-chain watcher the relay role uses to post foreign data (headers + inclusion proofs) to Settlement as `L1_RELAYED` account-blocks. Permissionless and trustless — the STF re-validates everything (§6.1, §4.2). Defined in `EXECUTOR.md` §7 (`Source`).
- **`BridgeLedger` (shared STF library).** The chain-neutral state machine (mint/burn/pool/conservation/peg-out/replay/fraud) a bridge domain's `Apply` delegates to; only the injected `ChainVerifier` and the asset differ between chains. A library, not a second Settlement.
- **`ReleaseAdapter` (write seam, on the foreign chain).** Consumes a finalized Zenon peg-out authorization and releases the real foreign asset. Settlement defines the authorization object and the obligations, **not** the adapter's code.

**Peg-out authorization (normative).** A bridge withdrawal is expressed as an ordinary `kind=1` outbox message (§17.2) — there is **no** per-chain withdrawal contract on Zenon. After finality the message is the authorization a `ReleaseAdapter` consumes on the foreign chain:

```
PegOutAuthorization := { domainId, outboxId, asset, amount, recipient(foreign), burnRef }
```

A conformant `ReleaseAdapter` deployment **MUST**: release at most once per `outboxId` (foreign-side replay protection mirroring `processedOutbox`, §17.3); verify Zenon-side finality/correctness appropriate to the domain's `execProfile` (a finalized-batch inclusion proof under `ATTESTATION`/`OPTIMISTIC`, a validity proof under `ZK_VALIDITY`); and release exactly `amount` of `asset` to `recipient`. The Zenon side **MUST NOT** depend on `ReleaseAdapter` liveness for safety — Settlement only emits the authorization; a withheld or buggy releaser delays but cannot over-release, since the authorization is one-per-`outboxId` and conservation-bounded (§18.3). Per-chain instantiations (BitVM graph / covenant script for BTC; verifier contract for EVM; program for SVM) and the exact proof object are out of scope here and specified per chain (companion `BRIDGE-FRAMEWORK-SPEC.md`).
...
- `total_size` **MUST** be the exact byte length and **MUST NOT** exceed `MaxCodeSize` (§25); `chunk_count` **MUST** equal `ceil(total_size / MaxDeployChunkData)` and **MUST NOT** exceed `MaxChunkCount`.
...
`DeployContractChunk(domainId, code_hash, chunk_index, chunk_hash, chunk_data)` — **Normative:** the record **MUST** exist (non-finalized, non-expired) and the submitting `caller` **MUST** equal the recorded deployer; `chunk_index ∈ [0, chunk_count)` and not already present; `chunk_data` ≤ `MaxDeployChunkData`, all chunks except the last exactly `MaxDeployChunkData`, last exactly `total_size - (chunk_count-1)*MaxDeployChunkData`; `chunk_hash` **MUST** equal `SHA3-256(chunk_data)`, verified by Settlement at the receive — the chunk bytes are in hand there, and this is Settlement's only byte-level verification of code. When `chunk_count == 1` the single chunk **is** the last chunk and its size **MUST** equal `total_size` (not `MaxDeployChunkData`). On success, record `(chunk_index → chunk_hash)`; chunk bytes are published in the data bundle, only the hash is committed to state.

> `MaxDeployChunkData` (§25) is sized below `MaxDataLength` so a full chunk **plus the method's ABI envelope** (selector, `domainId`, `code_hash`, `chunk_index`, `chunk_hash`, dynamic-bytes framing — ~260 bytes) always fits a single account-block's data field. `MaxDAChunkSize` governs only off-chain DA bundle chunk serving (§20) and plays no role in on-chain deployment.

### 11.3 DeployContractFinalize

`DeployContractFinalize(domainId, code_hash)` — **Normative:**

- **Settlement (on-chain):** the record **MUST** exist and the `caller` **MUST** be the deployer; all chunks **MUST** be present. On acceptance the record is consumed (finalized). Settlement performs **no** assembly, validation, or instrumentation, and never stores assembled bytecode; its per-chunk hash checks (§11.2) are its only byte-level code verification.
- **Runtime/STF (off-chain, deterministic):** bytecode **MUST** be assembled by concatenating chunks in ascending index; `SHA3-256(assembled)` **MUST** equal `code_hash` and `len` **MUST** equal `total_size`, else the input's receipt is `VALIDATION_FAILED` and no contract is created.
- Validation **MUST** then run on the assembled bytecode (runtime/STF): Core 1.0 structural validation; rejection of WASI/floats/threads/atomics/SIMD and post-MVP integer ops (sign-extension, `trunc_sat`) per §7.2; host-import restriction to exactly `storage_len`/`storage_read`/`abort`; size checks; required exports (§27.1).
- Instrumentation **MUST** then run deterministically (runtime/STF): gas-metering injection, stack-depth limiter, memory-growth limiter. The instrumented size **MUST NOT** exceed the post-instrumentation ceiling (§25).
- The executable `CodeHash` **MUST** be `SHA3-256(instrumented_bytecode)`, derived by the runtime. `ContractID` **MUST** be derived per §12 using the deployer address, this input's `globalInputIndex`, and `chain_id`. The deployment `ExecutionResult` **MUST** include both `code_hash` and `CodeHash`.

> *Informative.* The split follows from what each side can verify: Settlement holds chunk bytes only transiently (one receive at a time) and commits only their hashes, so the whole-assembly check runs in the runtime — where it is deterministic, reproducible by any watcher from L1-anchored data, and fraud-provable in Phase 2. A `code_hash` mismatch therefore consumes the record on-chain but deterministically creates no contract; the deployer restarts with a fresh record.
...
- `OWNER_UPGRADEABLE` contracts **MUST** specify an owner; `UpgradeContract` **MUST** be authorized by the owner (verified via `caller`, §9.2) and **MUST** produce a new `CodeHash`. Ownership transfer **MUST** be possible only by the current owner.
- **Upgrade staging and finalization.** Replacement bytecode **MUST** be staged through the chunked mechanism (§11.1–§11.2; the single-chunk flow covers small modules). `UpgradeContract(contract_id, new_code_hash)` is the **finalizer** of an upgrade-destined record: a complete, non-expired pending record for `(domainId, new_code_hash)` **MUST** exist and its recorded deployer **MUST** equal the upgrade `caller`; Settlement verifies these and consumes the record (the §11.3 on-chain half). The runtime/STF runs the full §11.3 assembly/validation/instrumentation pipeline: on success the contract's code is replaced (`CodeHash` changes, `ContractID` does not); on any pipeline failure the receipt is `VALIDATION_FAILED` with no code change. `DeployContractFinalize` and `UpgradeContract` are mutually exclusive consumers of a pending record — whichever is accepted first consumes it; `DeployContractFinalize` deploys a **new** contract (new `ContractID`), `UpgradeContract` re-codes an **existing** one. Expiry (§11.5) applies to upgrade-destined records identically.
...
**Normative:** an outbox message (encoding §27.4) contains source `contractId`, `inputIndex`, monotonic `outboxIndex`, `kind` (0 = L2 / cross-domain / cross-chain **message** delivery, 1 = L1 / foreign **withdrawal (peg-out) authorization**), the kind-specific target (target `ContractID`, or recipient address + `AssetID` + amount, where "recipient" is an L1 address for a Zenon-native withdrawal or a foreign-chain address for a bridge peg-out, §6.6), and a `payload` ≤ `MaxOutboxPayloadSize`. The per-input `outboxRoot` and the batch `outboxRoot` (over all messages ordered by `(inputIndex, outboxIndex)`) are Merkle roots included in the batch commitment.

A `kind=1` message is the single authorization object for *every* withdrawal — a Zenon-native ZTS release (consumed on Zenon by `Update`, §18.2) and a bridge peg-out (consumed on the foreign chain by a `ReleaseAdapter`, §6.6) are the same object with different consumers; there is no per-chain withdrawal path on Zenon. **`MESSAGING` domains MUST emit only `kind=0`; `BRIDGE` domains MAY emit both; `EXECUTION` domains emit `kind=0` for cross-contract/cross-domain delivery and `kind=1` for native ZTS withdrawals (§6.5).**
...
- `RelayMessage` is permissionless and **registers** a message; it releases no funds itself (§18.2, §27.9). It **MUST** supply the message and an inclusion proof against the committed `outboxRoot` of a `FINALIZED` batch (§21); Settlement **MUST** verify the proof and `FINALIZED` status before acting and **MUST** record the `outboxId` at registration — a message is registered at most once. **Settlement MUST additionally verify that the `domainId` and `batchId` supplied with the relay message match the domain and batch whose committed `outboxRoot` is used to verify the inclusion proof**; a relay presenting a valid proof against a mismatched domain or batch cursor **MUST** be rejected.
- For L1 withdrawals, registration enqueues the release (§18.2); release is performed by `Update` under the conservation rules (§18.3), bounded per call (§27.9).
...
### 17.4 Timeouts and acknowledgements (reserved)

**Normative (reserved, gated):** Phase 1 outbox is fire-and-forget with replay protection only. A generalized messaging profile — used by `MESSAGING` domains and optionally by `BRIDGE`/`EXECUTION` cross-chain delivery — **MAY** attach a `timeout` (a destination height/time or source height deadline) and an `ackPolicy` (`NONE` / `ON_DELIVERY` / `ON_EXECUTION`) to a message, with a `MessageState` per `outboxId` (§5.4). When present: the destination **MUST** refuse delivery past the timeout, and the source becomes eligible for a **refund/rollback** on a proof of non-delivery by the deadline; the timeout path and delivery **MUST** be mutually exclusive (the `outboxId` is consumed by whichever lands first); an ack is itself a `kind=0` message replay-protected by its own `outboxId`. Timeout and ack windows are pre-mainnet per channel and **MUST** be ordered against source and destination finality so no refund races a valid delivery (§21). None of this is activated while the Phase-1 profile is in force. Full lifecycle in the companion `BRIDGE-FRAMEWORK-SPEC.md`.

---

## 18. Asset model and conservation

### 18.1 Custody modes and asset identity

**Normative:** each `(domain, asset)` has a **custody mode** (§6.5):

- **`ESCROW`** — a real ZTS asset held 1:1 in Settlement custody. **The only mode activated in Phase 1.** Aggregate solvency is enforced on-chain independently of executor honesty (§18.3).
- **`MINTED`** (reserved, gated) — a representation asset (e.g. `zBTC`) whose supply is minted against attested foreign-chain locks and burned to authorize foreign release; it has no L1 deposit event behind it, so its conservation is enforced differently and its activation is gated on the §18.3 profile floor and reconciliation mechanism (OD-1).

**Phase 1 therefore MUST NOT issue `MINTED` representation assets; all Phase-1 execution-layer assets MUST be `ESCROW`, backed 1:1 by L1 assets in Settlement custody.** An `AssetID` **MUST** be the canonical 32-byte encoding of a 10-byte ZTS identifier: bytes `[0:22]` (the leading 22 bytes) are zero, and bytes `[22:32]` are the ZTS identifier in its native byte order. This map is bijective and is the **only** valid `AssetID` encoding; it **MUST NOT** be a hash of the ZTS and **MUST NOT** use any alternative alignment (e.g. the ZTS bytes at `[0:10]`). Settlement **MUST** reject any `AssetID` with a non-zero byte in `[0:22]` (`UNSUPPORTED_OPERATION` on user/executor inputs, `VALIDATION_FAILED` on `SubmitBatch`) and **MUST** reject any non-canonical alignment. All conservation counters (§18.3) and `AssetFlowSummary` entries (§18.4, §27.6) are keyed by this canonical `AssetID`, so the single encoding guarantees one custody pot per real asset.
...
- On `RelayMessage` of a withdrawal from a `FINALIZED` batch, Core **MUST** increase `pendingWithdrawalReserve[domainId][asset]` and append the withdrawal to the domain's release queue. `FINALIZED` status is the **only** time gate: registration and release add no further delay beyond §21. Release is performed by the permissionless, rate-limited **`Update`** method (§5.3): each accepted `Update` **MUST** release queued withdrawals in registration order, at most `MaxWithdrawalsPerUpdate` per call (§27.9), and for each release **MUST** move the amount into `totalReleased[domainId][asset]`, decrease `pendingWithdrawalReserve`, transfer the asset via a descendant send, and re-check the conservation invariant. `Update` follows the established embedded crank pattern — permissionless, rejected when called again within `UpdateMinNumMomentums` (`checkAndPerformUpdate`, `vm/embedded/implementation/common.go`; precedent `UpdateEmbeddedAcceleratorMethod`).

### 18.3 Conservation invariant

**Custody mode `ESCROW` (Phase 1).** For every `ESCROW` (domain, asset), after every state-modifying Settlement operation the following **MUST** hold, and Core **MUST** revert any operation that would violate it:
...
**Custody mode `MINTED` (representation asset, reserved — gated on the resolution below of OD-1).** For a `MINTED` (domain, asset), two coupled invariants **MUST** hold after every mutating operation:
...
`provenForeignLocks` increases only on a finalized, replay-checked, `ChainVerifier`-verified foreign lock; `mintedSupply` increases by the minted amount, never exceeding `provenForeignLocks`. `burnedSupply` increases on a finalized burn; `authorizedForeignRelease` increases on the `kind=1` peg-out authorization, never exceeding `burnedSupply`; a foreign release consumes exactly one `outboxId` (§17.3). Mint and burn **MUST** be gated on finalized batches only.

**The reconciliation gap (OD-1) and its resolution.** `ESCROW` backing is an L1 `Deposit` event Core reconciles against natively (§18.4); a `MINTED` mint is backed by a *foreign* lock verified inside the STF, which Core never sees, so Core cannot reconcile a mint by deposit-event correspondence. The predicates above are well-defined safety properties, but the layer that *enforces* them depends on where the backing fact first becomes Core-checkable. Core **MUST** reject `custodyMode = MINTED` unless at least (a)+(b) hold:

- **(a) Economic floor.** A per-(domain, asset) `MaxBatchMint` cap (§25, Core hard-bounded, symmetric to `MaxBatchWithdrawal`) **MUST** bound per-batch mint exposure, and the executor bond **MUST** be sized to cover it (§22). This bounds over-mint loss to a bond-covered amount.
- **(b) Optimistic enforcement.** `MINTED` **MUST** have `execProfile ≥ OPTIMISTIC` (§6.3): because the foreign-lock check is part of the STF, an unbacked mint is ordinary state divergence a watcher fraud-proves, rejecting the batch and slashing. Pure `ATTESTATION` **MUST NOT** back a `MINTED` asset's aggregate solvency.
- **(c) Core-direct enforcement (strongest, optional).** When `foreignProfile ∈ {COMMITTEE, ZK_CONSENSUS}` — a foreign fact Core can cheaply verify — Core **MAY** maintain `provenForeignLocks` itself by verifying the quorum signature or succinct proof at `SubmitBatch`, restoring the `ESCROW`-equivalent honesty-independent guarantee. This is unavailable under `LIGHT_CLIENT` (too heavy for Core), which is why `LIGHT_CLIENT` `MINTED` relies on (a)+(b).

> The honest statement: **`MINTED` aggregate solvency is only as strong as the layer that makes the foreign backing fact checkable to Core** — bond-bounded and fraud-proven under `LIGHT_CLIENT` + `OPTIMISTIC`, honesty-independent under `COMMITTEE`/`ZK_CONSENSUS`. The `MINTED` `AssetFlowSummary` reconciliation path (mint credits against finalized peg-ins, burn debits against finalized peg-outs) replaces §18.4's L1-deposit-event reconciliation; specifying its exact on-chain form is tracked as OD-1 (§30) and **MUST** be resolved before any `MINTED` asset is registered.
...
- **Automatic refund of the unclaimed remainder.** After execution the runtime **MUST** refund `deposit_amount − claimed_deposit` (on `SUCCESS`) or the full `deposit_amount` (on any failure — the contract's effects, including any claim, are discarded per §14.3) to the depositor. The refund is a runtime-emitted `L1_WITHDRAWAL` outbox message (§27.4) with `recipient_l1 = the source account-block's L1 sender`, `asset_id = deposit_asset`, `amount = the unclaimed remainder`; it is appended to the input's outbox set and committed in `outboxRoot`, then registered by `RelayMessage` once its batch is `FINALIZED` and released by `Update`, conservation-bounded (§18.3) and replay-protected like any withdrawal (§17.3). No refund outbox is emitted when the remainder is `0`.
...
**Size/bounds:** a commitment **MUST** fit `MaxOnChainCommitmentSize` (15 KiB, §25 — sized so the commitment plus its ABI envelope fits `MaxDataLength`) — roots and bounded metadata only. A batch **MUST NOT** exceed `MaxBatchInputs` and **MUST** be contiguous per domain (§4.6).
...
**SubmitBatch (normative):** only an **ACTIVE** (registered + fully bonded, §22) member of the domain's `executors` set that is the **entitled proposer** for the batch's `firstInputSeq` cursor position under the domain's `proposerPolicy` (§6.2) **MAY** submit. Core **MUST** verify: caller is an ACTIVE set member and the entitled proposer for `domainId` at that cursor position; `protocol_version` recognized; per-domain contiguity against the stored input-sequence cursor and Settlement's own assigned input indices (§4.6, incl. `lastInputSeq` not exceeding the highest assigned index); commitment size; `assetFlowSummary` deposit correspondence against the per-(domain, asset) cumulative deposit totals (§4.6, §18.4); `proofData` is the zero-length encoding; and `preStateRoot == previous accepted postStateRoot` for that domain. On acceptance the batch enters `SUBMITTED` and the domain's cursor advances to `lastInputSeq`. Core **MUST NOT** verify execution correctness in Phase 1. (Under the Phase-1 profile — `SINGLE`, a size-1 set — this reduces to "caller is the domain's single registered executor.")
...
- A batch enters `SUBMITTED` on acceptance; no withdrawal **MAY** be released from a `SUBMITTED` batch. It transitions to `FINALIZED` when the withdrawal delay (measured in L1 momentum heights from `submittedAtHeight`, not wall-clock) has elapsed. Withdrawals **MAY** be released only from `FINALIZED` batches; `FINALIZED` is the only time gate — registration (§17.3) and the `Update` release crank (§18.2) add no additional delay.
...
- The executor **MUST** post a bond before activation. The bond **MUST** be slashable on a successful fraud proof (Phase 2 / `OPTIMISTIC`) and **MUST** be sized to at least cover the **Core-ceiling value of `MaxBatchWithdrawal`** for the domain — and, for a `MINTED` (domain, asset), additionally the Core-ceiling value of `MaxBatchMint` (§18.3) — not the current Periphery-configured values. An administrator raising a cap toward the Core ceiling must not silently under-collateralize the executor; the bond is sized against the worst-case the administrator can configure within bounds, so that the constraint holds independently of any future Periphery change. Amounts are pre-mainnet parameters (§25.2).
- **Bond posting (sentinel flow).** The bond is two-legged — QSR and ZNN — posted by the registered executor address in two transactions, mirroring sentinel registration: (1) `DepositQsr` accumulates the QSR leg against the sender, reclaimable via `WithdrawQsr` while unconsumed; (2) `PostBond(domainId)` carries exactly the ZNN leg as the attached amount and consumes the required QSR deposit. `PostBond` **MUST** be accepted only from the domain's registered executor address (`RegisterExecutor` binds identity first) and only when the accumulated QSR deposit covers the required amount. The executor is **ACTIVE** — eligible to `SubmitBatch` (§19) — only while registered **and** fully bonded.
- **Bond return.** `RevokeBond` **MUST** be accepted only from an address that is no longer the domain's registered executor and only when every batch that address submitted is `FINALIZED`; it returns both legs in full to that address (no slashing exists in Phase 1).
...
**Normative:** the following components **MUST** carry explicit `u16` version fields, each first in its canonical encoding: `protocol_version` (batch commitments), `runtime_profile_version`, `metering_version` (Phase 1 = `1`, bound to `wasm-gas-table-v1.json`), `serialization_version`, `statediff_version`, `executionresult_version`, `receipt_version`, `outbox_version`, `damode_version`. `proofData` (§19) is empty under `ATTESTATION`-single-bonded and `OPTIMISTIC`-at-submit, carries a fraud proof (`OPTIMISTIC` `Challenge`) or a validity proof (`ZK_VALIDITY`) under the respective execution profile (§6.3), and **MUST** be empty in Phase 1.

The semantics-defining domain fields of §6 (`domainClass`, `inputSource`, `execProfile`, `foreignProfile`, and the identity portion of `chainBinding`) are folded into `stfSpecHash`; changing any is a runtime-semantics change under `MinRuntimeUpgradeDelay` (§6). A commitment-format change that places a quorum attestation in `proofData` under an `ATTESTATION`-quorum sub-variant **MUST** be accompanied by a `protocol_version` bump and **MUST NOT** be activated while Phase-1 `proofData` rules are in force.
...
| `MaxOnChainCommitmentSize` (sized so commitment + ABI envelope fits `MaxDataLength`) | 15 KiB (`1024 * 15`) |
| `MaxCallPayload` | 8 KiB |
| `MaxReturnData` | 4 KiB |
| `MaxExecutionResult` | 32 KiB |
| `MaxBatchInputs` | 64 |
| `MaxDAChunkSize` (off-chain DA bundle chunk serving only, §20) | 16 KiB |
| `MaxDeployChunkData` (on-chain deploy chunk payload, §11.2 — sized so chunk + ABI envelope fits `MaxDataLength`) | 15 KiB (`1024 * 15`) |
| `MaxCodeSize` (pre-instrumentation, runtime-neutral) | 256 KiB |
| `MaxChunkCount` | 18 |
...
| Executor bond ZNN + QSR amounts | Two-leg sentinel-style bond (§22); combined value at least the Core-ceiling `MaxBatchWithdrawal`, per domain. Natural reference: `SentinelZnnRegisterAmount`/`SentinelQsrDepositAmount` (`vm/constants/embedded.go`). |
| `MaxBatchWithdrawal` | Within Core floor/ceiling, per (domain, asset). |
| `MaxBatchMint` (reserved; `MINTED` only, §18.3) | Per-batch mint cap per (domain, asset), Core floor/ceiling, symmetric to `MaxBatchWithdrawal`; bounds over-mint exposure and sizes the bond (§22). Not used while the Phase-1 profile is in force. |
...
> **Foreign-recipient variant (reserved, gated).** The `kind==1` branch above encodes `recipient_l1 : Address(20)` — a Zenon-native withdrawal. A `BRIDGE` peg-out (§6.6) targets a foreign-chain address of chain-specific width; its encoding is a reserved, version-gated `kind==1` sub-variant (a `recipient : Bytes` form selected by the domain's `chainBinding`) accompanied by a `protocol_version`/`outbox_version` bump. It **MUST NOT** be emitted while the Phase-1 profile is in force; Phase-1 `kind==1` messages are Zenon-native `Address(20)` withdrawals only.
...
**Normative:** `RelayMessage` releases no funds — it registers exactly one message per call (§17.3, §18.2). A single `Update` **MUST** release at most `MaxWithdrawalsPerUpdate = 16` withdrawals (one descendant send each), draining the domain's release queue in registration order. Independently, a single batch **MUST NOT** register withdrawals whose aggregate per (domain, asset) exceeds `MaxBatchWithdrawal`. Both bounds **MUST** hold: the first bounds descendant-block fan-out per L1 action; the second bounds value at risk per batch.
...
**Normative:** `PAUSE_SUBMIT` halts `SubmitBatch`; `PAUSE_RELAY` halts `RelayMessage` registration and `Update` release. Activation **MUST** by default set `PAUSE_SUBMIT` only. `RelayMessage` and `Update` for withdrawals from already-`FINALIZED` batches **MUST** remain processable while only `PAUSE_SUBMIT` is active — halting submission **MUST NOT** freeze withdrawals already past the delay. `PAUSE_RELAY` **MUST** be a separately authorized escalation for an active asset-safety exploit, emitting a distinct public L1 event. Neither scope **MUST** ever rewrite history or alter roots/receipts/balances/conservation counters/canonical order. On `PAUSE_RELAY` deactivation, queued withdrawals **MUST** again be releasable in unchanged registration order.
...
- **Bonded registration + time-windows** — `RegisterSentinelMethod` and the `TimeChallenge` / `SecurityInfo` family (`vm/embedded/implementation/sentinel.go`, `common.go`) as templates for the executor bond and withdrawal delay. The two-leg bond (§22) reuses the sentinel QSR-deposit flow: `DepositQsrMethod`/`WithdrawQsrMethod` and `checkAndConsumeQsr` (`vm/embedded/implementation/common.go`), exact-amount ZNN leg per `RegisterSentinelMethod.ValidateSendBlock`.
- **Permissionless rate-limited crank** — the `Update` release method (§18.2) follows `UpdateEmbeddedAcceleratorMethod` / `checkAndPerformUpdate` (`vm/embedded/implementation/accelerator.go`, `common.go`): anyone may call, rejected within `UpdateMinNumMomentums` of the last accepted call, pays out via descendant sends.
...
**Bridge framework (schema present, activation reserved).** The generalized domain model (§6), the two profile ladders (§6.3, §6.4), custody modes and the `MINTED` conservation predicates (§18), the peg-out authorization + `ReleaseAdapter` obligations (§6.6), and the reserved timeout/ack messaging (§17.4) are normative here but constrained to their Phase-1 defaults until a spork-gated Core release activates them. The reference material — worked peg-in/peg-out examples per chain, comparative mappings (IBC/LayerZero/Wormhole/CCIP/Axelar/XCM/zkBridge), and the testing/formal-verification/deployment strategy — lives in the companion `research/bridges/BRIDGE-FRAMEWORK-SPEC.md`. Activation prerequisites: the fraud-proof referee registry (`OPTIMISTIC`), the validity/consensus verifier registries (`ZK_VALIDITY`/`ZK_CONSENSUS`), representation-asset (`MINTED`) issuance, and the per-chain `ChainVerifier`/`SourceAdapter`/`ReleaseAdapter` (the last off-Zenon).

**OPEN DESIGN — OD-1 (`MINTED` reconciliation), gating.** The `MINTED` conservation *predicates* are defined (§18.3), but the exact on-chain mechanism by which Core *verifies* them against committed batches — the `MINTED` analogue of §18.4's L1-deposit-event reconciliation, for which no L1 deposit event exists — is not yet specified. The resolution direction is fixed (§18.3: economic floor `MaxBatchMint` + `OPTIMISTIC` execution floor, optionally Core-direct under `COMMITTEE`/`ZK_CONSENSUS`), but the concrete `AssetFlowSummary`-side reconciliation format and the Core→verifier dispatch interface (shared with the reserved verifier registries, §5.4) remain OPEN and **MUST** be resolved before any `MINTED` asset is registered. This is the single gating blocker for asset bridges.
...
> **⚠️ Artifact status.** The shipped `execution-conformance-v1.json` (EXEC-001…012) does not match this specification: its frame/blob layout lacks `deposit_asset`/`deposit_amount`/`claimed_deposit` (§18.5, §27.1, §27.1a). The EXEC-DEP/EXEC-FAIL-OUTBOX and CV-* vectors below (including CV-PATH/CV-APPLY in `smt-v1-test-vectors.json`) are not yet generated. The suites **MUST** be regenerated against this spec version before stage-1A conformance gating (§29); until then only SMT-001…014 are authoritative. See `EXECUTOR.md` P-7.
...
- **Settlement storage key layout (§5.4):** domain registry, per-(domain, asset) counters, input-index assignment records, the withdrawal release queue, processed-outbox set, deploy records.
