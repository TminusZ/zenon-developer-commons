...
- **Domain-agnostic core** — one implementation serving every domain: the domain registry, per-(domain, asset) escrow/custody and the conservation invariant, executor registration + bond, batch-commitment chaining/contiguity keyed by `domainId`, input-index assignment at receive (SPEC §4.6), the withdrawal delay/finality state machine, the release queue drained by the permissionless `Update` crank (SPEC §18.2), outbox replay protection, and pause. This is the only code that moves value.
...
3. **State (SMT)** — the executor's own leaf set over the shared path-based core now shipped in `common/trie` (SPEC §13.1). Produces `preStateRoot`/`postStateRoot` per input and witnesses every read.
...
The **watcher** role runs stages 1–4 identically (through the batch builder's root assembly) and compares its recomputed roots against the committed batch instead of submitting (Phase 2: emitting a fraud proof); the **relay** role, used only by external-input domains (§6 below, SPEC §6.1), posts source data to L1 as account-blocks and runs no STF. See `EXECUTOR.md`.
...
**Inputs and ordering.** L2 inputs are ordinary account-blocks sent to Settlement (data + plasma only). The sequencer queues sends per-contract and the VM consumes them in canonical order; the canonical order is exactly the committed `MomentumContent` order — ascending `AccountHeader.Bytes() = Address‖Height‖Hash` (`common/types/account_header.go`, `chain/nom/momentum_content.go`). The executor reproduces this with no new derivation, so it has zero sequencing authority and force-inclusion is native (SPEC §4). Settlement reproduces the same numbering on-chain: embedded receives replay the committed send order (the per-contract sequencer is FIFO — `chain/account/sequencer.go`), so the contract assigns each input's index at its receive and verifies batch bounds against its own count (SPEC §4.6).

**Value movement.** Embedded receives auto-process (pillars auto-receive, `pillar/worker_contract_generator.go`) and can emit descendant `ContractSend` blocks (`vm/vm.go`) — the mechanism by which withdrawal release pays out: `RelayMessage` registers a proven withdrawal, and the permissionless, rate-limited `Update` crank (the Accelerator-payout pattern — `UpdateEmbeddedAcceleratorMethod`, `checkAndPerformUpdate`) releases due entries via descendant sends (SPEC §18.2). Embedded addresses have unlimited L1 plasma; users pay for inputs via fused plasma or PoW. DA is inherited because inputs are on-chain account-blocks, capped at 16 KiB (`vm/constants/plasma.go`).
...
Anyone ──RelayMessage{message, inclusion proof vs outboxRoot}──▶ Settlement
  Settlement: verify proof + FINALIZED + outboxId not in processedOutbox; register pendingWithdrawalReserve; enqueue
Anyone ──Update──▶ Settlement: release queued withdrawals in registration order (≤ MaxWithdrawalsPerUpdate)
  via descendant ContractSend, re-checking conservation per release
```

`RelayMessage` and `Update` are permissionless; `RelayMessage` registers (it never pays), and each `Update` releases at most `MaxWithdrawalsPerUpdate` queued withdrawals (bounded descendant fan-out, SPEC §27.9). FINALIZED is the only time gate on release. Replay is impossible via the unique `outboxId` + `processedOutbox` set.

### 4.3 Contract deployment

```
DeployContractStart(code_hash, total_size, chunk_count) → pending record keyed by (domainId, code_hash)
DeployContractChunk × chunk_count (≤ MaxDeployChunkData = 15 KiB each, hash-committed —
                                    chunk + ABI envelope fits the 16 KiB account-block data limit)
...
Content-addressing by the pre-instrumentation `code_hash` removes all executor discretion over the assembled bytecode (SPEC §11). Two hashes: `code_hash` (developer identity) vs `CodeHash` (executable identity). An `OWNER_UPGRADEABLE` upgrade stages its replacement code through the same chunk mechanism; `UpgradeContract` — not `DeployContractFinalize` — finalizes an upgrade-destined record, running the identical validation/instrumentation pipeline against the existing `ContractID` (SPEC §12).

### 4.4 Bridge peg-in / peg-out (reserved — schema-present, gated)

A `BRIDGE` domain reuses the exact same machinery; the only new logic is the `ChainVerifier` inside the STF and the foreign-side `ReleaseAdapter`. Illustrative for a `MINTED` asset (e.g. `zBTC`), inactive until the profiles activate (SPEC §6, §18.3):

```
PEG-IN (foreign → Zenon):
  User: lock BTC on Bitcoin (to a ReleaseAdapter-governed output), tagging the Zenon recipient
  Relayer (permissionless, SourceAdapter): post BTC header(s) + inclusion proof to Settlement (L1_RELAYED account-block)
  Executor STF: ChainVerifier verifies PoW/linkage/finality + Merkle inclusion; BridgeLedger credits provenForeignLocks,
                emits mint effect (mintedSupply += amount ≤ provenForeignLocks); replay-keyed by the foreign outpoint
  Settlement: SubmitBatch → … FINALIZED → zBTC minted to recipient
PEG-OUT (Zenon → foreign):
  User: burn zBTC (BridgeLedger debits; burnedSupply += amount) → emit kind=1 peg-out authorization
  Settlement: … FINALIZED → authorization is a committed kind=1 outbox object
  Relayer: carry {authorization, proof} to the foreign ReleaseAdapter
  ReleaseAdapter (off-Zenon): verify the finalized-batch proof, release exactly amount to recipient, once per outboxId
```

Peg-out is the same `kind=1` object as a native withdrawal (§4.2) with a foreign consumer instead of `Update`. Conservation for `MINTED` is enforced by the profile floor (`OPTIMISTIC` fraud proofs + `MaxBatchMint`, or Core-direct under `COMMITTEE`/`ZK_CONSENSUS`); the exact reconciliation is OD-1 (SPEC §18.3, §30). Worked per-chain examples: `research/bridges/BRIDGE-FRAMEWORK-SPEC.md`.
...
The core hashes, routes, and proves identically for both — the same vector set (`common/trie/testdata/smt-v1-test-vectors.json`, gated by `conformance_test.go`) governs it. The adapters differ only in how they map keys to paths and how they treat empties. The L2 adapter **must not** reuse the L1 adapter, for the two reasons in SPEC §13.1: the L1 maintenance adapter (`tree.go`) pre-hashes its db key, so feeding it an already-32-byte L2 path would double-hash; and it folds an empty value to a deletion (`stagedApplier` in `common/trie/momentum_fold.go`), whereas L2 requires a *present-empty* leaf distinct from absence (SPEC §13.6). The executor therefore uses only the path-native API now exported by `common/trie` — `RootOfLeaves`/`ProveByPath`/`VerifyProofByPath`/`VerifyAbsenceByPath` (`pathapi.go`, `proof.go`) — and never the key-hashing `Tree`/`VerifyProof` path. The payoff: in Phase 2, the on-chain dispute referee verifies L2 proofs with the *same* `VerifyProofByPath` the executor uses — one verifier, no second implementation.

---

## 6. Domain model and isolation

Settlement anchors **domains**; a domain is `{domain class, runtime, STF spec, input source, execution profile, foreign-fact profile, bonded executor set, state-root lineage, optional foreign-chain binding}`. Per-domain keying on every counter is what gives **domain isolation**: a faulty or dishonest domain's executor can never reach another domain's custody, because conservation is enforced per (domain, asset) and the executor is bound per `domainId`.
...
**Domain class, profiles, and bridges (SPEC §6.3–§6.6).** A domain is one of `EXECUTION` (computation, e.g. WASM — Phase 1), `BRIDGE` (moves value across a foreign chain, with custody), or `MESSAGING` (moves cross-chain data only, no custody). Two **orthogonal** security ladders make trust a per-domain choice: an **execution** ladder (`ATTESTATION → OPTIMISTIC → ZK_VALIDITY` — the Phase 1→2→3 ladder lifted per-domain) securing the batch computation, and a **foreign-fact** ladder (`NONE → COMMITTEE → LIGHT_CLIENT → ZK_CONSENSUS`) securing the truth of an external event. A bridge is still the same four-method plugin: its `Apply` delegates to a shared **`BridgeLedger`** (mint/burn/pool/conservation/peg-out) with a per-chain **`ChainVerifier`** (reads foreign facts), fed by a **`SourceAdapter`** (relay role) and consumed on the far side by a **`ReleaseAdapter`** (releases the real asset, off-Zenon). A withdrawal — Zenon-native or bridge peg-out — is the *same* `kind=1` outbox object with a different consumer; there is no per-chain withdrawal path on Zenon. Each `(domain, asset)` carries a custody mode: `ESCROW` (real ZTS in custody — Phase 1) or `MINTED` (a representation asset, e.g. `zBTC`, backed by attested foreign locks — reserved, gated on OD-1). All of this is schema-present and reserved; Phase 1 activates only the `EXECUTION`/`ATTESTATION`/`NONE`/`ESCROW`/`SINGLE` point. Reference material: `research/bridges/BRIDGE-FRAMEWORK-SPEC.md`.

---

## 7. Trust and safety model

Phase 1 is **bonded attestation, not trustless execution** (SPEC §1) — the single base point of a general trust surface. A domain's trust is two **independent** dimensions: an **execution profile** (how the batch computation is secured) and a **foreign-fact profile** (how an external event's truth is secured), and **a domain is exactly as trustless as the weaker of the two**. At the Phase-1 point (`ATTESTATION` / `NONE` / `ESCROW`) the on-chain, executor-independent guarantee is aggregate `ESCROW` solvency per (domain, asset): `totalReleased + pendingWithdrawalReserve ≤ totalDeposited`; the safety mechanisms are the executor bond, the withdrawal delay, the per-batch cap, the conservation invariant, and public commitment visibility. Per-account correctness relies on executor honesty until `OPTIMISTIC` fraud proofs; the foreign-fact profile removes the analogous trust on external inputs. For `MINTED` representation assets the aggregate guarantee is profile-dependent — only as strong as the layer that makes the foreign backing fact checkable to Core (SPEC §18.3). This framing must be communicated to users without obfuscation.

---

## 8. Forward compatibility

The Phase 1 surface is chosen so Phases 2 and 3, and the bridge framework, are additive:

- **Phase 2 (fraud proofs).** Adds the momentum state root (the L1 half of the shared SMT, already in progress via `StateRootSpork`), a per-call replay referee that verifies L2 proofs with `VerifyProofByPath`, a dispute-game contract with slashing, DA availability enforcement, inter-batch outbox ordering, and activation of the permissioned executor set. The `DomainRecord` already carries the `executors` set, `proposerPolicy`, and `inputSource`, and the commitment the `executorId`; the per-domain input-sequence contiguity already pins the challenge target. Phase 2 *populates* these fields rather than adding them — nothing in the on-chain footprint changes except added Periphery verifier registries.
...
- **Bridge framework (per-domain, chain-neutral).** The generalized domain model already carries `domainClass`, `execProfile`, `foreignProfile`, `chainBinding`, and per-asset custody mode (SPEC §6, §18), so a Bitcoin/Ethereum/Solana bridge or a pure messaging channel is a new plugin (`ChainVerifier` + `SourceAdapter` + `ReleaseAdapter` + asset wiring), not a new settlement layer. A `MESSAGING` domain (no custody, no OD-1 dependency) is the cheapest first activation of the `L1_RELAYED` + `ChainVerifier` + relay/watcher machinery; trust-minimized `MINTED` asset bridges follow `OPTIMISTIC` fraud proofs (SPEC §18.3). Activation is spork-gated; nothing in the Phase-1 footprint changes.

The decision that makes all of this possible is taken in Phase 1: **inputs stay as on-chain account-blocks**, so DA is inherited from L1 and every input is replayable by anyone — including relayed foreign-chain headers and proofs, which is exactly what lets a bridge be a plugin rather than a protocol change.

---

*See `SPEC.md` for all normative requirements (the shared SMT is specified in §13 and implemented in `common/trie`), `EXECUTOR.md` for the executor binary architecture (relay/executor/watcher roles, codebase grounding), `../merkle-state-root/MERKLE_STATE_ROOT.md` for the L1 momentum state root, and `../phase-2/SPEC.md` / `../phase-3/SPEC.md` for later phases.*
