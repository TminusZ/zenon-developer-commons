...
**Document status:** Companion specification for a generalized, multi-chain Bridge Framework. RFC 2119 / RFC 8174 language throughout.
**Adoption note (SPEC v1.6.0):** the framework's **core normative model** has been absorbed into `SPEC.md` as first-class schema — the generalized `DomainRecord` and domain classes (SPEC §6), the two profile ladders (SPEC §6.3–§6.4), legal combinations and custody modes (SPEC §6.5), the `ChainVerifier`/`SourceAdapter`/`BridgeLedger`/`ReleaseAdapter` abstractions and the `kind=1` peg-out authorization (SPEC §6.6), generalized messaging (SPEC §17), and the `ESCROW`/`MINTED` conservation model incl. the OD-1 resolution (SPEC §18.3). **`SPEC.md` governs on any conflict.** This document is retained as the **companion** carrying what was deliberately not inlined: the worked per-chain examples (Appendix B), the comparative mappings to external systems (Appendix C), the full Assumptions Register, and the testing / formal-verification / deployment strategy (§16–§20). Where a core-model section here overlaps a now-normative `SPEC.md` section, the `SPEC.md` text is authoritative and this document is background.
...
- `SPEC.md` v1.6.0: off-chain WASM execution + the absorbed generalized domain/bridge model, bonded attestation, on-chain conservation. The normative contract for every consensus-visible byte; **governs on any conflict**.
- `ARCHITECTURE.md`: design companion (informative); §4.4/§6–§8 carry the generalized domain, bridge flows, and two-ladder trust model.
- `EXECUTOR.md` v0.3.0: executor binary architecture (relay / executor / watcher roles, plugin seam, `BridgeLedger`/`ChainVerifier` delegation).
