...
> **⚠️ Status: STALE — do not gate an implementation on these vectors.**
> This artifact does not match the current `../SPEC.md`: the `ExecutionInputFrame` here lacks
> `deposit_asset`/`deposit_amount` (§27.1) and the `ContractEffects`/`ExecutionResult` blobs end at
> `ReturnData` with no `claimed_deposit` (§27.1a) — under the current spec every vector's blob is a
> `RUNTIME_FAULT`. The §31 vector set (EXEC-DEP-*, EXEC-FAIL-OUTBOX-1, CV-BUNDLE/ASSET/COMMIT/
> INPUTROOT/OUTBOX/DEPLOY, and the CV-PATH/CV-APPLY trie vectors) is not yet generated.
> Regenerate against the current `../SPEC.md` version before build stage 1A conformance gating
> (`../SPEC.md` §29, `../EXECUTOR.md` P-7).
